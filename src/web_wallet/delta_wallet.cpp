...
std::string delta_wallet::get_commands_str()
{
  std::stringstream ss;
  ss << "Commands: " << ENDL;
...
bool delta_wallet::viewkey(const std::vector<std::string> &args/* = std::vector<std::string>()*/)
{
...
bool delta_wallet::spendkey(const std::vector<std::string> &args/* = std::vector<std::string>()*/)
{
...
bool delta_wallet::seed(const std::vector<std::string> &args/* = std::vector<std::string>()*/)
{
...
bool delta_wallet::help(const std::vector<std::string> &args/* = std::vector<std::string>()*/)
{
...
{
  m_cmd_binder.set_handler("start_mining", boost::bind(&delta_wallet::start_mining, this, _1), "start_mining [<number_of_threads>] - Start mining in daemon");
  m_cmd_binder.set_handler("stop_mining", boost::bind(&delta_wallet::stop_mining, this, _1), "Stop mining in daemon");
  m_cmd_binder.set_handler("save_bc", boost::bind(&delta_wallet::save_bc, this, _1), "Save current blockchain data");
  m_cmd_binder.set_handler("refresh", boost::bind(&delta_wallet::refresh, this, _1), "Resynchronize transactions and balance");
  m_cmd_binder.set_handler("balance", boost::bind(&delta_wallet::show_balance, this, _1), "Show current wallet balance");
  m_cmd_binder.set_handler("incoming_transfers", boost::bind(&delta_wallet::show_incoming_transfers, this, _1), "incoming_transfers [available|unavailable] - Show incoming transfers - all of them or filter them by availability");
  m_cmd_binder.set_handler("payments", boost::bind(&delta_wallet::show_payments, this, _1), "payments <payment_id_1> [<payment_id_2> ... <payment_id_N>] - Show payments <payment_id_1>, ... <payment_id_N>");
  m_cmd_binder.set_handler("bc_height", boost::bind(&delta_wallet::show_blockchain_height, this, _1), "Show blockchain height");
  m_cmd_binder.set_handler("transfer", boost::bind(&delta_wallet::transfer, this, _1), "transfer <mixin_count> <addr_1> <amount_1> [<addr_2> <amount_2> ... <addr_N> <amount_N>] [payment_id] - Transfer <amount_1>,... <amount_N> to <address_1>,... <address_N>, respectively. <mixin_count> is the number of transactions yours is indistinguishable from (from 0 to maximum available)");
  m_cmd_binder.set_handler("set_log", boost::bind(&delta_wallet::set_log, this, _1), "set_log <level> - Change current log detalization level, <level> is a number 0-4");
  m_cmd_binder.set_handler("address", boost::bind(&delta_wallet::print_address, this, _1), "Show current wallet public address");
  m_cmd_binder.set_handler("save", boost::bind(&delta_wallet::save, this, _1), "Save wallet synchronized data");
  m_cmd_binder.set_handler("viewkey", boost::bind(&delta_wallet::viewkey, this, _1), "Get viewkey");
  m_cmd_binder.set_handler("spendkey", boost::bind(&delta_wallet::spendkey, this, _1), "Get spendkey");
  m_cmd_binder.set_handler("seed", boost::bind(&delta_wallet::seed, this, _1), "Get deterministic seed");
  m_cmd_binder.set_handler("help", boost::bind(&delta_wallet::help, this, _1), "Show this help");
}
//----------------------------------------------------------------------------------------------------
bool delta_wallet::set_log(const std::vector<std::string> &args)
{
  if(args.size() != 1)
  {
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::ask_wallet_create_if_needed()
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::init(const boost::program_options::variables_map& vm)
{
...
//----------------------------------------------------------------------------------------------------
void delta_wallet::handle_command_line(const boost::program_options::variables_map& vm)
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::try_connect_to_daemon()
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::new_wallet(const string &wallet_file, const std::string& password, const crypto::secret_key& recovery_key, bool recover, bool two_random)
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::open_wallet(const string &wallet_file, const std::string& password)
{
  m_wallet_file = wallet_file;
  m_wallet.reset(new tools::wallet2());
  m_wallet->callback(this);
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::close_wallet()
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::save(const std::vector<std::string> &args)
{
  try
  {
    m_wallet->store();
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::start_mining(const std::vector<std::string>& args)
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::stop_mining(const std::vector<std::string>& args)
{
  if (!try_connect_to_daemon())
    return true;
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::save_bc(const std::vector<std::string>& args)
{
  if (!try_connect_to_daemon())
    return true;
...
//----------------------------------------------------------------------------------------------------
void delta_wallet::on_new_block(uint64_t height, const cryptonote::block& block)
{
  m_refresh_progress_reporter.update(height, false);
}
//----------------------------------------------------------------------------------------------------
void delta_wallet::on_money_received(uint64_t height, const cryptonote::transaction& tx, size_t out_index)
{
...
//----------------------------------------------------------------------------------------------------
void delta_wallet::on_money_spent(uint64_t height, const cryptonote::transaction& in_tx, size_t out_index, const cryptonote::transaction& spend_tx)
{
...
//----------------------------------------------------------------------------------------------------
void delta_wallet::on_skip_transaction(uint64_t height, const cryptonote::transaction& tx)
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::refresh(const std::vector<std::string>& args)
{
  if (!try_connect_to_daemon())
    return true;
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::show_balance(const std::vector<std::string>& args/* = std::vector<std::string>()*/)
{
  success_msg_writer() << "balance: " << print_money(m_wallet->balance()) << ", unlocked balance: " << print_money(m_wallet->unlocked_balance());
  return true;
}
//----------------------------------------------------------------------------------------------------
bool delta_wallet::show_incoming_transfers(const std::vector<std::string>& args)
{
  bool filter = false;
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::show_payments(const std::vector<std::string> &args)
{
  if(args.empty())
  {
...
//----------------------------------------------------------------------------------------------------
uint64_t delta_wallet::get_daemon_blockchain_height(std::string& err)
{
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::show_blockchain_height(const std::vector<std::string>& args)
{
  if (!try_connect_to_daemon())
    return true;
...
//----------------------------------------------------------------------------------------------------
bool delta_wallet::transfer(const std::vector<std::string> &args_)
{
  if (!try_connect_to_daemon())
    return true;
...
//----------------------------------------------------------------------------------------------------
void delta_wallet::stop()
{
  m_cmd_binder.stop_handling();
  m_wallet->stop();
}
//----------------------------------------------------------------------------------------------------
bool delta_wallet::print_address(const std::vector<std::string> &args/* = std::vector<std::string>()*/)
{
  success_msg_writer() << m_wallet->get_account().get_public_address_str();
  return true;
}
//----------------------------------------------------------------------------------------------------
bool delta_wallet::process_command(const std::vector<std::string> &args)
{
...
int main(int argc, char* argv[])
{
  ...
  web_wallet::rpc_server::init_options(desc_params);
